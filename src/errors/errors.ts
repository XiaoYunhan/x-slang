...
export class ConstNotInitialize extends RuntimeSourceError {
  constructor(node: es.Node, private name: string) {
    super(node)
  }

  public explain() {
    return `Missing initializer in const declaration of constant ${this.name}.`
  }

  public elaborate() {
    return `As ${this.name} is a constant value, it should be initialized when declaring it.`
  }
}

export class ArrayIndexError extends RuntimeSourceError {
  constructor(node: es.Node, private prop: Value) {
    super(node)
  }

  public explain() {
    return `Type ${this.prop} cannot be used as an index type of Array.`
  }
