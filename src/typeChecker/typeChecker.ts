...
function lookupDeclKind(name: string, env: Env) {
  for (let i = env.length - 1; i >= 0; i--) {
...
function _infer(
  ...
  switch (node.type) {
    ...
    case 'BinaryExpression': {
      ...
      if (opType === undefined) return constraints
    ...
    case 'WhileStatement': {
      ...
      let whileConstraints = addToConstraintList(constraints, [storedType, bodyType])
      try {
        whileConstraints = infer(testNode, env, whileConstraints)
        whileConstraints = addToConstraintList(whileConstraints, [testType, tBool])
      } catch (e) {
        console.log(e)
      }
      return infer(bodyNode, env, whileConstraints, isTopLevelAndLastValStmt)
    }
    case 'ForStatement': {
    ...
    case 'IfStatement': {
      ...
      let altType: any
      try {
        altNode = node.alternate as TypeAnnotatedNode<es.Node>
        altType = altNode.inferredType as Variable
      } catch {}
    ...
    // throw Error('Arrow functions not supported for x-slang')
    case 'VariableDeclaration': {
      const initNode: any = node.declarations[0].init!
      const identifierNode: any = node.declarations[0].id!
      let variableConstraints = addToConstraintList(constraints, [storedType, tUndef])
      variableConstraints = infer(initNode, env, variableConstraints)
      if (identifierNode.typeAnnotation !== undefined) {
        const annotationType: any = getTypeFromIdentifier(identifierNode)
        const assignedType = typeof initNode.value
        if (annotationType.name !== typeof initNode.value) {
          throw Error(
            `The type of assigned value doesn't match the annotation type: ${assignedType} assigned to ${annotationType.name}`
          )
        }
        variableConstraints = addToConstraintList(variableConstraints, [
          annotationType,
          initNode.inferredType as Variable
        ])
        setType(identifierNode.name, annotationType, env)
      } else {
        setType(identifierNode.name, initNode.inferredType as Variable, env)
      }
    ...
    case 'FunctionDeclaration': {
      ...
      paramNodes.forEach((paramNode: any) => {
        if (paramNode.typeAnnotation === undefined) {
          ...
          const annotationType = getTypeFromIdentifier(paramNode)
          setType(paramNode.name, annotationType, env)
        }
      })
      const result = infer(bodyNode, env, newConstraints)
      env.pop()
    ...
    case 'AssignmentExpression': {
      ...
      const rightNode = node.right as TypeAnnotatedNode<es.Identifier>
      const rightType = rightNode.inferredType as Variable
      const leftType = leftNode.inferredType as Variable
      let leftActualType: any = undefined
      let rightActualType: any = undefined
      if (leftNode.type === 'Identifier') {
        leftActualType = lookupType(leftNode.name, env)
      }
      if (node.right.type === 'Literal') {
        rightActualType = typeof node.right.value
      } else if (node.right.type === 'Identifier') {
        rightActualType = lookupType(rightNode.name, env)
        rightActualType = rightActualType.name
      }
      if (leftActualType !== undefined && rightActualType !== undefined) {
        if (leftActualType.name !== rightActualType) {
          throw Error(
            `The type of assigned value doesn't match the annotation type: ${leftActualType.name} assigned to ${rightActualType}`
          )
        }
      }
    ...
    case 'ArrayExpression': {
      let arrayConstraint = constraints
      const elements = node.elements as TypeAnnotatedNode<es.Node>[]
      elements.forEach(element => {
        arrayConstraint = infer(element, env, arrayConstraint)
      })
      const arrayType = elements.map(
        element => applyConstraints(element.inferredType as Variable, arrayConstraint) as Type
      )
      const inferredArrayType = tList(arrayType[0])
      arrayConstraint = addToConstraintList(arrayConstraint, [storedType, inferredArrayType])
...
function getTypeFromBabelTSType(tsType: babel.types.TSType): Type {
  if (tsType.type === 'TSNumberKeyword') return tNumber
  else if (tsType.type === 'TSStringKeyword') return tString
  else if (tsType.type === 'TSArrayType')
    return tList(getTypeFromBabelTSType(tsType.elementType) as Type)
  else if (tsType.type === 'TSFunctionType') {
    const paramType = tsType.parameters.map(param =>
      getTypeFromIdentifier(param as babel.types.Identifier)
    )
    const returnType = getTypeFromTSTypeAnnotation(
      (tsType as babel.types.TSFunctionType).typeAnnotation as babel.types.TSTypeAnnotation
    )
    return tFunc(...paramType, returnType)
  }
  throw Error('unexpected TSType')
}

function getTypeFromIdentifier(identifier: babel.types.Identifier): Type {
  const annotation: any = identifier.typeAnnotation
  return getTypeFromBabelTSType(annotation.typeAnnotation)
}

function getTypeFromTSTypeAnnotation(annotation: babel.types.TSTypeAnnotation): Type {
  return getTypeFromBabelTSType(annotation.typeAnnotation)
}

function setDeclKind(name: string, kind: any, env: Env) {
...
// function tArray(...types: Type[]): SArray {
//   return {
//     kind: 'array'
//     elementType: tUnion(...types)
//   }
// }
